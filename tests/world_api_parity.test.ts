...
//
// `IWorld` (src/world_api.ts:341-510, 153 members) is the ONE seam render/ui depend
...
// it lands the matching edit here in the SAME commit. The count pins (153 / 38 / 115)
...
// The 153 members of `interface IWorld`, in interface order (world_api.ts:342-509).
// Partition: 38 `data` + 115 `method` (read-returning + command-void + 3 async).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  ...
  it('the full sorted member set is exactly the pinned 153', () => {
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 20 facets', () => {
  ...
  it('the union of the 20 facets equals the pinned 153-member IWORLD_MEMBERS set', () => {
    const union = Object.values(FACET_MEMBER_ARRAYS).flatMap((arr) => [...arr]);
    expect(union.length, 'union size before dedup (catches a duplicated member)').toBe(153);
    expect(new Set(union).size, 'union size after dedup (catches a duplicated member)').toBe(153);
