 ...
 *
 * `game` is the housekeepingSummary Pick rather than the whole GameServer so
 * BOTH dispatch arms can call this one function: the legacy handleAdminApi
 * ladder passes the live game, and the migrated admin RouteDef handlers
 * (server/admin.ts) pass the boot-injected AdminRuntime, which is itself a
 * Pick of GameServer. Type-only narrowing; runtime behavior is unchanged.
 */
export async function handleHousekeepingApi(
  path: string,
  req: http.IncomingMessage,
  res: http.ServerResponse,
  game: Pick<GameServer, 'housekeepingSummary'>,
  adminAccountId: number,
): Promise<boolean> {
